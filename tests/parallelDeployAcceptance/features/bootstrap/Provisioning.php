...
trait Provisioning {
	...
	public function connectToLdap(array $suiteParameters):void {
		...
		$this->ldap->bind();
		if(isset($suiteParameters['ldapInitialUserFilePath'])){
			$this->importLdifFile(
	...
	public function generatedPwHash() {
		// 
	}
	...
	public function createLdapUser(array $setting):void {
		...
		if (OcisHelper::isTestingParallelDeploy()) {
			$uidNumber = \count($this->ldapCreatedUsers) + 20004;
			$entry['homeDirectory'] = '/home/' . $setting["userid"];
			$entry['objectclass'][] = 'organizationalPerson';
			$entry['objectclass'][] = 'ownCloud';
			$entry['objectclass'][] = 'person';
			$entry['objectclass'][] = 'top';
			$entry['gidNumber'] = 30000;
		...
		echo "User 'admin' exists in ldap: $ldapResponse\n";
	...
	public function deleteLdapUsersAndGroups():void {
		...
		foreach ($this->ldapCreatedUsers as $user) {
			$this->ldap->delete(
				"uid=" . $user . ",ou=" . $this->ldapUsersOU . "," . $this->ldapBaseDN,
				true
			);
	...
	public function userExists(?string $user):bool {
		...
		if (OcisHelper::isTestingOnOcisOrReva()) {
			...
			if (OcisHelper::isTestingOnOcis() && OcisHelper::isTestingParallelDeploy()) {
				$requestingUser = $user;
